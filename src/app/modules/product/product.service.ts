...
const ProductService = {
    ...
    async getAllProducts(query: Record<string, unknown>) {
        // Resolve category slug or ID to itself and all its descendants recursively
        const categoryFilter = query.category as string;
        if (categoryFilter) {
            let categoryDoc = null;
            if (mongoose.Types.ObjectId.isValid(categoryFilter)) {
                categoryDoc = await Category.findById(categoryFilter);
            } else {
                categoryDoc = await Category.findOne({ slug: categoryFilter, isDeleted: false });
            }

            if (categoryDoc) {
                const getDescendants = async (catId: string): Promise<string[]> => {
                    const children = await Category.find({ parent: catId, isDeleted: false, isActive: true }).select('_id');
                ...
                const descendantIds = await getDescendants(categoryDoc._id.toString());
                const allCategoryIds = [categoryDoc._id.toString(), ...descendantIds];
                query.category = { $in: allCategoryIds };
            } else {
                // Category not found, force empty result
